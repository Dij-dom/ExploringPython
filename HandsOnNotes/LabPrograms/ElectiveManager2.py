...
#  Calculating the grade of the student and storing in a new collection called
# results
...
def display_grade(StudentDataCS, StudentDataStats, name, mark):
    for name in StudentDataCS:
        ...
        results = dict({name,gradee})
    for name in StudentDataStats:
        if mark >80:
            gradee = "A"
            print(f"{name} got A grade.\n")
        elif (mark>60 and mark<=80):
